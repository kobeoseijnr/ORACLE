    ...
    for pref_idx, preference in enumerate(preferences):
        ...
        try:
            ...
            if hasattr(env.base_env, 'cur_specs'):
                ...
                # Capture final design parameters (indices + actual values)
                params_idx = None
                params_vals = None
                try:
                ...
                pareto_solutions.append({
                    ...
                    'steps': steps,
                    'design_params_idx': params_idx.tolist() if hasattr(params_idx, 'tolist') else (list(params_idx) if params_idx is not None else None),
                    'design_params': params_vals
                })
        except Exception as e:
    ...
    for target_idx in range(min(num_targets, num_specs)):
        ...
        try:
            ...
            for sol_idx, sol in enumerate(pareto_solutions):
                ...
                # Flatten design parameters into columns (both index and value)
                design_params_idx = sol.get('design_params_idx', None)
                design_params = sol.get('design_params', None)
                params_id = getattr(env.base_env, 'params_id', []) if hasattr(env, 'base_env') else []
                flat_param_cols = {}
                for i, pid in enumerate(params_id):
                    pid_str = str(pid)
                    flat_param_cols[f'param_idx_{pid_str}'] = (
                        int(design_params_idx[i]) if isinstance(design_params_idx, (list, tuple)) and i < len(design_params_idx) else None
                    )
                    if isinstance(design_params, dict) and pid_str in design_params:
                        flat_param_cols[f'param_{pid_str}'] = design_params.get(pid_str)
                    else:
                        flat_param_cols[f'param_{pid_str}'] = None
