...
def graph_1b_best20_pca_colored():
    """Best 20 PCA scatter with updates: one color per spec, ◇ Target (AutoCkt), ○ Target (MORL), ■ AUCRT, ▲ MORL."""
    df = pd.read_csv(BEST_20_CSV)
    df = df[df["spec"].astype(str) != "summary"]
    if len(df) == 0:
        print("SKIP colored PCA: no data in best_20_both.csv")
        return
    orig = df[df["method"] == "original"].reset_index(drop=True)
    morl = df[df["method"] == "morl"].reset_index(drop=True)
    target_orig = build_feature_matrix(orig, use_target=True)
    orig_mat = build_feature_matrix(orig, use_target=False)
    target_morl = build_feature_matrix(morl, use_target=True)
    morl_mat = build_feature_matrix(morl, use_target=False)
    all_mat = np.vstack([target_orig, orig_mat, target_morl, morl_mat])
    scaler = StandardScaler()
    X_scaled = scaler.fit_transform(all_mat)
    X_pca = PCA(n_components=2).fit_transform(X_scaled)
    pc1, pc2 = X_pca[:, 0], X_pca[:, 1]
    ...
    t_x, t_y = pc1_s[:n].copy(), pc2_s[:n].copy()
    o_x, o_y = pc1_s[n : 2 * n].copy(), pc2_s[n : 2 * n].copy()
    tm_x, tm_y = pc1_s[2 * n : 3 * n].copy(), pc2_s[2 * n : 3 * n].copy()
    m_x, m_y = pc1_s[3 * n :].copy(), pc2_s[3 * n :].copy()
    if np.mean(t_x) < np.mean(o_x):
        t_x, o_x, tm_x, m_x = 100 - t_x, 100 - o_x, 100 - tm_x, 100 - m_x
    if np.mean(o_y) < np.mean(m_y):
        t_y, o_y, tm_y, m_y = 100 - t_y, 100 - o_y, 100 - tm_y, 100 - m_y
    ...
    ax.set_xlabel("Value")
    ax.set_ylabel("Value")
    ax.set_title("Best 20 – Same color = Target + its Output pair\n◇ Target (AutoCkt) + ■ AutoCkt output  |  ○ Target (MORL) + ▲ MORL")
    ax.grid(True, alpha=0.3)
    ...
    for spec_idx in range(n):
        ...
        for obj_idx in range(4):
            ...
            ax.scatter(tx, ty, c=[c], s=40, marker="D", alpha=0.95, edgecolors="black", linewidths=0.5)
    ...
    for spec_idx in range(n):
        c = spec_colors[20 + spec_idx]
        for obj_idx in range(4):
            tx = tm_x[spec_idx] + obj_offset[obj_idx, 0] + offset_t[0]
            ty = tm_y[spec_idx] + obj_offset[obj_idx, 1] + offset_t[1]
    ...
    shape_handles = [
        Line2D([0], [0], marker="D", color="w", markerfacecolor="gray", markeredgecolor="black", markersize=10, label="Target (AutoCkt)"),
        Line2D([0], [0], marker="o", color="w", markerfacecolor="gray", markeredgecolor="black", markersize=10, label="Target (MORL)"),
        Line2D([0], [0], marker="s", color="w", markerfacecolor="gray", markeredgecolor="black", markersize=10, label="AutoCkt output"),
        Line2D([0], [0], marker="^", color="w", markerfacecolor="gray", markeredgecolor="black", markersize=10, label="MORL"),
    ]
...
def _plot_6subplots_merged(sample, title, filename, use_shuffled_outputs=False):
    """Plot 6 objective-pair subplots from merged data. One color per spec, consistent across all plots."""
    n = len(sample)
    sample = sample.copy()
    sample["tg_db"] = gain_linear_to_db(sample["target_gain_linear_orig"].values)
    sample["tu"] = sample["target_ugbw_mhz_orig"].astype(float)
    sample["tp"] = sample["target_pm_deg_orig"].astype(float)
    sample["ti"] = sample["target_ibias_ma_orig"].astype(float)
    ...
    def jitter_grid(n_pts, scale=0.018):
    ...
    jx_t, jy_t = jitter_grid(n)
    jx_o, jy_o = jitter_grid(n, 0.018)
    jx_m, jy_m = jitter_grid(n, 0.018)
    ...
    def plot_pair(ax, x_tgt, y_tgt, x_out, y_out, x_lbl, y_lbl, subplot_title):
        ...
        for i in range(n):
            c = spec_colors[i]
            ax.scatter(sample.iloc[i][x_tgt] + jx_t[i] * x_range, sample.iloc[i][y_tgt] + jy_t[i] * y_range,
                      c=[c], s=45, marker="o", alpha=0.95, edgecolors="black", linewidths=0.5)
            ax.scatter(sample.iloc[i][x_out] + jx_o[i] * x_range, sample.iloc[i][y_out] + jy_o[i] * y_range,
                      c=[c], s=45, marker="s", alpha=0.95, edgecolors="black", linewidths=0.5)
            ax.scatter(sample.iloc[i][x_out.replace("_orig", "_morl")] + jx_m[i] * x_range,
                      sample.iloc[i][y_out.replace("_orig", "_morl")] + jy_m[i] * y_range,
                      c=[c], s=45, marker="^", alpha=0.95, edgecolors="black", linewidths=0.5)
        ax.set_xlabel(x_lbl)
        ax.set_ylabel(y_lbl)
    ...
    pairs = [
        ("tg_db", "tp", "output_gain_db_orig", "output_pm_deg_orig", "Gain (dB)", "PM (°)", "PM vs Gain"),
        ...
        ("tu", "tp", "output_ugbw_mhz_orig", "output_pm_deg_orig", "UGBW (MHz)", "PM (°)", "UGBW vs PM"),
        ("tu", "ti", "output_ugbw_mhz_orig", "output_ibias_ma_orig", "UGBW (MHz)", "I-Bias (mA)", "UGBW vs I-Bias"),
        ("tp", "ti", "output_pm_deg_orig", "output_ibias_ma_orig", "PM (°)", "I-Bias (mA)", "PM vs I-Bias"),
    ]
    ...
    handles = [
        ...
        Line2D([0], [0], marker="s", color="w", markerfacecolor="gray", markeredgecolor="black", markersize=8, label="AutoCkt output"),
        Line2D([0], [0], marker="^", color="w", markerfacecolor="gray", markeredgecolor="black", markersize=8, label="MORL"),
    ]
...
def graph_analysis1_aucrt_targets_vs_morl_outputs():
    """Analysis 1: AUCRT input targets vs MORL outputs. Random 20 specs, same color per spec across all plots."""
    if not ORIGINAL_CSV.exists() or not MORL_BEST_1000.exists():
        print("SKIP analysis1: missing CSV")
        return
    df_orig = pd.read_csv(ORIGINAL_CSV)
    df_orig = df_orig[df_orig["spec"].astype(str) != "summary"]
    df_morl = pd.read_csv(MORL_BEST_1000)
    ...
    df_morl["spec_key"] = df_morl["spec"].astype(int) - 1
    merged = df_orig.merge(df_morl, on="spec_key", suffixes=("_orig", "_morl"))
    sample = merged.sample(n=min(20, len(merged)), random_state=SEED).reset_index(drop=True)
    _plot_6subplots_merged(sample,
        "Analysis 1: AUCRT Input Targets vs MORL Outputs\nRandom 20 specs — Same color = Target + AutoCkt output + MORL per spec",
        "analysis1_aucrt_targets_vs_morl_outputs.png")


def graph_analysis2_morl_targets_vs_aucrt_outputs():
    """Analysis 2: MORL input targets vs AUCRT outputs. MORL best 20 specs."""
    if not ORIGINAL_CSV.exists() or not MORL_BEST_1000.exists() or not BEST_20_CSV.exists():
        print("SKIP analysis2: missing CSV")
        return
    df_orig = pd.read_csv(ORIGINAL_CSV)
    df_orig = df_orig[df_orig["spec"].astype(str) != "summary"]
    df_morl = pd.read_csv(MORL_BEST_1000)
    ...
    df_morl["spec_key"] = df_morl["spec"].astype(int) - 1
    merged = df_orig.merge(df_morl, on="spec_key", suffixes=("_orig", "_morl"))
    morl_best = pd.read_csv(BEST_20_CSV)
    morl_best = morl_best[morl_best["method"] == "morl"]
    spec_ids = morl_best["spec"].astype(int).values
    spec_keys = spec_ids - 1
    sample = merged[merged["spec_key"].isin(spec_keys)].reset_index(drop=True)
    if len(sample) < 20:
        sample = merged[merged["spec_key"].isin(spec_keys)].head(20)
    _plot_6subplots_merged(sample,
        "Analysis 2: MORL Input Targets vs AUCRT Outputs\nMORL Best 20 specs — Same color = Target + AutoCkt output + MORL per spec",
        "analysis2_morl_targets_vs_aucrt_outputs.png")


def graph_analysis3_aucrt_targets_vs_random_outputs():
    """Analysis 3: AUCRT targets vs randomly sampled outputs (baseline)."""
    if not ORIGINAL_CSV.exists() or not MORL_BEST_1000.exists():
        print("SKIP analysis3: missing CSV")
        return
    df_orig = pd.read_csv(ORIGINAL_CSV)
    df_orig = df_orig[df_orig["spec"].astype(str) != "summary"]
    df_morl = pd.read_csv(MORL_BEST_1000)
    ...
    sample = merged.sample(n=min(20, len(merged)), random_state=SEED).reset_index(drop=True)
    _plot_6subplots_merged(sample,
        "Analysis 3: AUCRT Targets vs Random Outputs (Baseline)\nRandom 20 targets, outputs shuffled from other specs",
        "analysis3_aucrt_targets_vs_random_outputs.png",
        use_shuffled_outputs=True)
...
def main():
    ...
    graph_analysis1_aucrt_targets_vs_morl_outputs()
    graph_analysis2_morl_targets_vs_aucrt_outputs()
    graph_analysis3_aucrt_targets_vs_random_outputs()
    print("=" * 60)
    print("Done.")
    print("=" * 60)
