...
try:
    ...
    IPython = None  # Optional dependency for interactive debugging
import itertools
...
class TwoStageAmp(gym.Env):
    ...
    path = _ROOT_DIR
    CIR_YAML = os.path.join(_ROOT_DIR, 'eval_engines', 'ngspice', 'ngspice_inputs', 'yaml_files', 'two_stage_opamp.yaml')
    ...
    def __init__(self, env_config):
        ...
        if self.generalize == False:
            ...
            load_specs_path = os.path.join(TwoStageAmp._ROOT_DIR, "methodology", "autockt", "gen_specs", "ngspice_specs_gen_two_stage_opamp")
        ...
        # Initialize NGSpice simulation environment
        use_surrogate = os.environ.get('AUTOCKT_USE_SURROGATE', 'False').lower() == 'true'
        if use_surrogate:
            from eval_engines.ngspice.surrogate_wrapper import SurrogateTwoStageClass
            self.sim_env = SurrogateTwoStageClass(yaml_path=TwoStageAmp.CIR_YAML, num_process=1, path=TwoStageAmp.path)
        else:
...
#        self.cur_params_idx = self.cur_params_idx + np.array(self.action_arr[int(action)])
    ...
    def update(self, params_idx):
        ...
        result = self.sim_env.create_design_and_simulate(param_val[0])
        if isinstance(result, tuple) and len(result) >= 2:
            specs_dict = result[1]
        else:
            specs_dict = result
        
        if isinstance(specs_dict, OrderedDict):
            cur_specs = OrderedDict(sorted(specs_dict.items(), key=lambda k:k[0]))
            cur_specs = np.array(list(cur_specs.values()))
        else:
            cur_specs = np.array(list(specs_dict.values()) if hasattr(specs_dict, 'values') else specs_dict)
...
def main():
  ...
  if IPython is not None:
    ...
    print("IPython not available. Environment test completed.")
