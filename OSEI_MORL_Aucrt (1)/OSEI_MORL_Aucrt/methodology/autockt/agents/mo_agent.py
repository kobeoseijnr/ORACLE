...
def nw_scalarization_torch(q_values, preference):
    """NW (Normalized Weighted Sum) scalarization for Q-values (PyTorch version)."""
    pref_expanded = preference.unsqueeze(1)
    return torch.sum(q_values * pref_expanded, dim=2)
...
class MO_DQN_Agent:
    ...
    def select_action(self, state, preference=None, epsilon=None):
        ...
        if self.scalarization == 'nw':
            scalarized_q = nw_scalarization_torch(q_values, preference.unsqueeze(0))
        else:
    ...
    def update(self, batch_size=32):
        ...
        with torch.no_grad():
            ...
            if self.scalarization == 'nw':
                next_scalarized_q = nw_scalarization_torch(next_q_for_action, preferences)
            else:
