...
def convert_markdown_tables(html):
    ...
    for line in lines:
        ...
        is_table_row = (
            '|' in stripped
            and (stripped.startswith('|') or stripped.count('|') >= 2)
...
<body>
    ...
    return html_template.format(content=content, title="AutoCkt 20-Sample Analysis Report")

def create_html_document_with_title(content, title):
    """Create complete HTML document with styling."""
    html_document = create_html_document(content)
    return html_document.replace(">AutoCkt 20-Sample Analysis Report<", f">{title}<")

def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--input", default=str(MD_FILE))
    parser.add_argument("--output", default=str(HTML_FILE))
    parser.add_argument("--title", default="AutoCkt 20-Sample Analysis Report")
...
def main():
    ...
    html_document = create_html_document_with_title(html_content, args.title)
