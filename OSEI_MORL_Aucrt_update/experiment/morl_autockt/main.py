...
Runs all post-evaluation steps: processing, graph generation, and comparison
"""
...
def process_results(raw_results_path, target_specs_path, output_dir, scenario_name, target_type, eval_method):
    """Process MORL results for one scenario"""
    print(f"\n{'='*80}")
    ...
    for sol in raw_results['all_solutions']:
        ...
        target_array = [
            ...
            sol.get('target_pm_deg', target_data.get('target_pm_deg', 0)),
            target_data.get('target_ugbw_mhz', 0) * 1e6
        ]
        
        # Evaluate using MORL methodology
        preference = sol.get('preference', None)
        morl_reached, scalarized_value = check_target_reached_morl(
            actual_specs, target_array, specs_id, preference=preference, threshold=-0.456650
        )
        target_reached = 'Yes' if morl_reached else 'No'
        
        result_entry = {
            'spec': spec_num,
            'solution': sol.get('solution', 1),
            'target_gain_linear': sol.get('target_gain_linear', target_data.get('target_gain_linear')),
            'target_ugbw_mhz': sol.get('target_ugbw_mhz', target_data.get('target_ugbw_mhz')),
            'target_pm_deg': sol.get('target_pm_deg', target_data.get('target_pm_deg')),
            'target_ibias_ma': sol.get('target_ibias_ma', target_data.get('target_ibias_ma')),
    ...
    total = len(results)
    unique_specs = len(set(r['spec'] for r in results if r['target_reached'] == 'Yes'))
    
    print(f"Summary: {passed}/{total} ({passed/total*100:.2f}%) passed, {unique_specs} unique specs")
...
def main():
    ...
    if not RAW_RESULTS.exists():
        ...
        print("Please run evaluation first: python evaluate.py <model_path>")
        sys.exit(1)
    ...
    # Process all scenarios: Original/15% × MORL/Strict/Tolerance
    ORIGINAL_TARGETS = BASE_DIR / "data" / "target_specs_original.json"
    TARGETS_15PERCENT = BASE_DIR / "data" / "target_specs_15percent.json"
    
    if not TARGETS_15PERCENT.exists() and ORIGINAL_TARGETS.exists():
        print("\nCreating 15% increased targets...")
        with open(ORIGINAL_TARGETS, 'r') as f:
            original_data = json.load(f)
        targets_15percent = {}
        for key, data in original_data.items():
            targets_15percent[key] = {
                'target_gain_linear': data.get('target_gain_linear', 0) * 1.15,
                'target_ugbw_mhz': data.get('target_ugbw_mhz', 0) * 1.15,
                'target_pm_deg': data.get('target_pm_deg', 0) * 1.15,
                'target_ibias_ma': data.get('target_ibias_ma', 0) * 1.15
            }
        TARGETS_15PERCENT.parent.mkdir(parents=True, exist_ok=True)
        with open(TARGETS_15PERCENT, 'w') as f:
            json.dump(targets_15percent, f, indent=2)
    
    # Generate only 4 graphs: Original/15% × MORL/Tolerance
    # Scenario 1: Original + MORL
    if ORIGINAL_TARGETS.exists():
        csv_path = process_results(RAW_RESULTS, ORIGINAL_TARGETS, OUTPUT_DIR, "original_morl", "original", "morl")
        graph_path = GRAPHS_DIR / "morl_autockt_4_subplots_original_morl.png"
        generate_graphs(csv_path, graph_path, "original", "morl")
    
    # Scenario 2: Original + Tolerance
    if ORIGINAL_TARGETS.exists():
        csv_path = process_results(RAW_RESULTS, ORIGINAL_TARGETS, OUTPUT_DIR, "original_tolerance", "original", "tolerance")
        graph_path = GRAPHS_DIR / "morl_autockt_4_subplots_original_tolerance.png"
        generate_graphs(csv_path, graph_path, "original", "tolerance")
    
    # Scenario 3: 15% Increased + MORL
    if TARGETS_15PERCENT.exists():
        ...
        graph_path = GRAPHS_DIR / "morl_autockt_4_subplots_15percent_morl.png"
        generate_graphs(csv_path, graph_path, "15percent", "morl")
    
    # Scenario 4: 15% Increased + Tolerance
    if TARGETS_15PERCENT.exists():
        csv_path = process_results(RAW_RESULTS, TARGETS_15PERCENT, OUTPUT_DIR, "15percent_tolerance", "15percent", "tolerance")
        graph_path = GRAPHS_DIR / "morl_autockt_4_subplots_15percent_tolerance.png"
        generate_graphs(csv_path, graph_path, "15percent", "tolerance")
