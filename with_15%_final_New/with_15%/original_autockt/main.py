...
def main():
    ...
    parser.add_argument('--force', action='store_true', help='Force regeneration even if CSV already exists')
    ...
    def _csv_data_hash(filepath):
        """Compute SHA256 hash of data lines in CSV (excludes summary/checksum rows). Uses raw text for stability."""
        with open(filepath, 'r') as f:
            lines = f.readlines()
        header = lines[0]
        data_lines = [l for l in lines[1:] if not l.startswith('summary,') and not l.startswith('checksum,')]
        content = header + ''.join(data_lines)
        return hashlib.sha256(content.encode()).hexdigest()[:16]

    # Only strict evaluation with original targets.
    csv_path = OUTPUT_DIR / "original_autockt_results_original.csv"
    if csv_path.exists() and not args.force:
        print(f"Results already exist: {csv_path}")
        print("Use --force to regenerate. Skipping to avoid inconsistencies.")
        # Verify integrity if checksum row exists
        with open(csv_path, 'r') as f:
            last_lines = f.readlines()
        checksum_line = [l for l in last_lines if l.startswith('checksum,')]
        if checksum_line:
            stored_hash = checksum_line[0].split(',')[2] if len(checksum_line[0].split(',')) > 2 else ''
            computed_hash = _csv_data_hash(csv_path)
            if stored_hash.strip() == computed_hash:
                print(f"Integrity check PASSED (sha256: {computed_hash})")
            else:
                print(f"WARNING: Integrity check FAILED. Stored={stored_hash.strip()}, Computed={computed_hash}")
                print("Data may have been modified. Use --force to regenerate.")
        else:
            print("No checksum found in CSV. Use --force to regenerate with checksum.")
    elif GEN_SPECS.exists():
        csv_strict = process_results(PICKLE_REACHED, PICKLE_NREACHED, OUTPUT_DIR, 'strict', 'original',
                                     gen_specs_path=GEN_SPECS)
        if csv_strict:
            # Add checksum row for integrity verification
            data_hash = _csv_data_hash(csv_strict)
            with open(csv_strict, 'a') as f:
                # Append checksum row: spec=checksum, evaluation_method=sha256, target_reached=hash
                n_cols = len(open(csv_strict).readline().split(','))
                row_parts = ['checksum', 'sha256', data_hash] + [''] * (n_cols - 3)
                f.write(','.join(row_parts) + '\n')
            print(f"Added integrity checksum: {data_hash}")
