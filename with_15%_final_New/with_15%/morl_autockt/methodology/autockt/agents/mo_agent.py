...
from NW_COSINE import (
    cosine_similarity_scalarization,
    normalized_weighted_sum_scalarization,
    scalarize_reward,
    cosine_similarity_scalarization_torch,
    normalized_weighted_sum_scalarization_torch,
    scalarize_q_torch,
)
...
class MO_PPO_Agent:
    ...
    def __init__(self, state_dim, action_dim, reward_dim, device='cpu', lr=3e-4, scalarization="cosine"):
    ...
    def compute_advantages(self, rewards, values, dones):
        ...
        for t in reversed(range(len(rewards))):
            ...
            if len(rewards[t].shape) > 0:  # Vector reward
                scalar_reward = scalarize_reward(rewards[t], self.current_preference.cpu().numpy(), method=self.scalarization)
                scalar_value = scalarize_reward(values[t], self.current_preference.cpu().numpy(), method=self.scalarization)
                scalar_next_value = scalarize_reward(next_value, self.current_preference.cpu().numpy(), method=self.scalarization)
...
class MO_DQN_Agent:
    ...
    def __init__(self, state_dim, action_dim, reward_dim, device='cpu', lr=1e-3, scalarization="cosine"):
        self.state_dim = state_dim
        self.action_dim = action_dim
        self.reward_dim = reward_dim
        self.device = device
    ...
    def select_action(self, state, preference=None, epsilon=None):
        ...
        scalarized_q = scalarize_q_torch(q_values, preference.unsqueeze(0), method=self.scalarization)
    ...
    def update(self, batch_size=32):
        ...
        with torch.no_grad():
            ...
            next_scalarized_q = scalarize_q_torch(next_q_for_action, preferences, method=self.scalarization)
...
class MO_DQN_LLM_Agent(MO_DQN_Agent):
    """
    Multi-Objective DQN Agent with LLM-based action masking.
    Inherits from MO_DQN_Agent and adds:
      - Action masking via LLMActionFilter (invalid actions get Q = -inf)
      - Dynamic preference boosting based on LLM advice
    """

    def __init__(self, state_dim, action_dim, reward_dim, llm_filter,
                 device='cpu', lr=1e-3, scalarization="cosine",
                 use_preference_boost=True):
        super().__init__(state_dim, action_dim, reward_dim, device, lr, scalarization)
        self.llm_filter = llm_filter
        self.use_preference_boost = use_preference_boost
        self._current_mask = None
        self._current_boost = "none"

    def update_llm_mask(self, current_specs, target_specs):
        """Query the LLM filter and store the current mask + boost."""
        mask, boost = self.llm_filter.get_action_mask_and_preference(
    ...
    def get_boosted_preference(self, preference):
        """Apply LLM preference boost if enabled."""
        if self.use_preference_boost and self._current_boost != "none":
            return self.llm_filter.apply_preference_boost(
                np.array(preference), self._current_boost
            )
        return np.array(preference)

    def select_action(self, state, preference=None, epsilon=None):
        """Select action with LLM action masking applied."""
        if preference is None:
            preference = self.current_preference
        else:
            preference = torch.FloatTensor(preference).to(self.device)
        ...
        if mask is not None:
            valid_actions = [i for i in range(self.action_dim) if mask[i]]
        else:
        ...
        if mask is not None:
            sq = scalarized_q.squeeze(0).clone()
            for i in range(self.action_dim):
                if not mask[i]:
                    sq[i] = float('-inf')
            return sq.argmax().item()
