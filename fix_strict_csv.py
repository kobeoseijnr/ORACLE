...
path = r'C:\Users\kobeo\OneDrive\Desktop\trail\with_15%_with_20\with_15%\morl_autockt\results\morl_autockt_results_original_strict.csv'
...
# Recalculate target_reached with STRICT checks
df['target_reached'] = df.apply(
    lambda r: 'Yes' if (r['output_gain_linear'] >= r['target_gain_linear'] and
                        r['output_ugbw_mhz'] >= r['target_ugbw_mhz'] and
                        r['output_pm_deg'] >= r['target_pm_deg'] and
                        r['output_ibias_ma'] <= r['target_ibias_ma']) else 'No', axis=1)

n = len(df)
cp = (df['target_reached'] == 'Yes').sum()
avg_fom = df['fom'].mean()
