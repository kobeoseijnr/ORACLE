...
def run_evaluation(
    ...
    # Load target specs JSON for reference
    with open(target_specs_path, 'r') as f:
        target_specs_json = json.load(f)
    print(f"Loaded {len(target_specs_json)} target specifications from JSON")
    ...
    for target_idx in range(min(num_targets, num_specs)):
        ...
        try:
            ...
            if force_target_pm_deg is not None:
                # TwoStageAmp specs order: ['gain_min','ibias_max','phm_min','ugbw_min']
                # so phase margin is index 2
                if len(target_spec) > 2:
                    target_spec[2] = float(force_target_pm_deg)
            ...
            for sol_idx, sol in enumerate(pareto_solutions):
                ...
                results['all_solutions'].append({
                    ...
                    'target_pm_deg': float(force_target_pm_deg) if force_target_pm_deg is not None else target_data.get('target_pm_deg'),
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run MORL evaluation")
    parser.add_argument("model_path", type=str, help="Path to trained MORL model checkpoint")
    parser.add_argument("--num_targets", type=int, default=1000, help="Number of targets to evaluate (default: 1000)")
    parser.add_argument("--num_preferences", type=int, default=10, help="Number of preference vectors per target (default: 10)")
    parser.add_argument("--max_steps", type=int, default=120, help="Maximum steps per preference (default: 120)")
    parser.add_argument("--dataset", type=str, default=None, help="Path to dataset (default: auto-detect)")
    parser.add_argument("--target_specs", type=str, default=None, help="Path to target specs JSON (default: auto-detect)")
    parser.add_argument(
        "--use_surrogate",
        type=str,
        default=None,
        help="Set AUTOCKT_USE_SURROGATE for this run: true/false. If omitted, respects existing environment variable.",
    )
    parser.add_argument(
        "--force_target_pm_deg",
        type=float,
        default=None,
        help="If set, overrides the target phase margin (phm_min) used during evaluation.",
    )
    
    args = parser.parse_args()

    if args.use_surrogate is not None:
        os.environ['AUTOCKT_USE_SURROGATE'] = str(args.use_surrogate).strip().lower()
    ...
    if args.dataset:
        ...
        DATASET_PATH = BASE_DIR.parent.parent / "dataset" / "ngspice_specs_gen_two_stage_opamp"
        if not DATASET_PATH.exists():
            DATASET_PATH = BASE_DIR / "autockt" / "gen_specs" / "ngspice_specs_gen_two_stage_opamp"
        if not DATASET_PATH.exists():
            DATASET_PATH = BASE_DIR / "methodology" / "autockt" / "gen_specs" / "ngspice_specs_gen_two_stage_opamp"
    
    if args.target_specs:
        TARGET_SPECS_PATH = Path(args.target_specs)
    else:
        TARGET_SPECS_PATH = BASE_DIR / "data" / "target_specs_original.json"
        if not TARGET_SPECS_PATH.exists():
            print(f"ERROR: Target specs not found: {TARGET_SPECS_PATH}")
            print("Please provide --target_specs or ensure target_specs_original.json exists")
            sys.exit(1)
    ...
    run_evaluation(
        ...
        OUTPUT_DIR,
        args.num_targets,
        args.num_preferences,
        args.max_steps,
        force_target_pm_deg=args.force_target_pm_deg,
    )
